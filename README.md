...
Chat in the middle. The UI talks to native `fx acp`, so tokens and tool calls stream live. While the agent runs, a quiet activity trail shows what it is doing (reads, commands, waits) and how long it has been working — so a long turn never looks stuck. A collapsible **Files** sidebar on the right is the workspace the agent sees — click a file to edit it in that same pane, chevron to go back to the tree.
