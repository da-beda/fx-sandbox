...
def run_fxs(ws: str, fx_args: list[str], perm: str = "yolo", timeout: int = 90) -> subprocess.CompletedProcess:
    ...
    cmd = [bin_, "run", "-w", ws, "--perm", perm, "--"] + fx_args
    env = os.environ.copy()
    env["FX_MODEL"] = MODEL
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if u.path == "/api/status":
            ...
            self._json(200, {
                ...
                "docker": docker_state(),
                "fxs": bool(which("fxs")),
                "perm": os.environ.get("FX_PERMISSION_MODE", "yolo"),
            })
            return
    ...
    def _fx(self, payload: dict) -> None:
        ...
        if not isinstance(args, list) or not args:
            ...
            return
        cmd0 = str(args[0])
        if cmd0 not in SAFE_FX and cmd0 not in ("pr", "issue"):
            self._json(400, {"error": "command not allowed"})
            return
        extra = [str(a)[:200] for a in args[1:8]]
        fx_args = [cmd0] + extra
        prompt = str(payload.get("prompt") or "").strip()
        if cmd0 in ("pr", "issue") and prompt:
            fx_args += ["--", prompt]
        if demo_mode():
            self._json(200, {"ok": True, "text": f"demo: fx {' '.join(fx_args)}"})
            return
        try:
            ...
            return
        perm = str(payload.get("perm") or "yolo")
        if perm not in ("ask", "auto", "yolo"):
            perm = "yolo"
        try:
        ...
        self._json(200, {"ok": r.returncode == 0, "text": text.strip()[-8000:], "code": r.returncode})
    ...
    def _ask(self, payload: dict) -> None:
        ...
        self._run_fxs(
            ...
            str(payload.get("perm") or ("yolo" if payload.get("yolo", True) else "auto")),
    ...
    def _run_fxs(self, prompt: str, ws: str, resume: str, perm: str, images: list, emit) -> None:
        ...
        if perm not in ("ask", "auto", "yolo"):
            perm = "yolo"
        cmd = [bin_, "run", "-w", ws, "--perm", perm, "--", "ask", "--json"]
        if perm == "yolo":
            cmd.append("--yolo")
        if resume:
            cmd += ["--resume", resume]
