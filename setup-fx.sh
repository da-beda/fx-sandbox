...
download_and_install_fx() {
  ...
  if [[ "$dest" == /usr/local/bin || "$dest" == /usr/bin ]]; then
    ...
    mkdir -p "$dest"
    install -m 0755 "${tmp}/fx" "${dest}/fx"
  fi
  ok "installed $("${dest}/fx" --version 2>/dev/null || echo fx) -> ${dest}/fx"
  rm -rf "$tmp"
  trap - EXIT
}
...
write_kit() {
  ...
  # tiny aliases that call THIS script if we have a real path; otherwise
  # the generated run-fx.sh is already standalone.
  if ! is_piped; then
    local src="${BASH_SOURCE[0]:-}"
    if [[ -n "$src" && -f "$src" && "$src" != "${dest}/setup-fx.sh" ]]; then
      cp "$src" "${dest}/setup-fx.sh"
      chmod 0755 "${dest}/setup-fx.sh"
    fi
  fi
...
${C_GRN}${LOG_PREFIX} done.${C_OFF}
  ...
  # native (no export needed if the key was saved)
  fx ask --no-save "Reply with: GLM52_OK"

  # sandboxed (only \$PWD is visible)
  setup-fx.sh run
